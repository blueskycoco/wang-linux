...
static int __init omap3stalker_model_setup(char *str)
{
	...
	if (MODEL_STRLENGTH == strlen(str)) {
		...
		if (match) {	/*decode other part */
			...
			if ((board_model == SBC35X_A2)||(board_model ==SBC35X_B1)) {
				switch (tmp_char) {
				case 'A':
					break;
				case 'N':
...
#define OMAP3STALKER_ETHR_GPIO_IRQ	186
#define OMAP3STALKER_SMC911X_CS	3
...
/*--------------------------dual uart support for EGS35X------------------------*/
#define STALKER_16C2550_CSA 3
#define STALKER_16C2550_CSB 4
#define STALKER_SERIAL_GPIO_IRQ_A 27
#define STALKER_SERIAL_GPIO_IRQ_B 26
#define STALKER_16C2550_CLK		1843200

#define PORT(_base,_irq)				\
	{						\
		.mapbase		= _base,		\
		.irq		= _irq,			\
		.uartclk	= STALKER_16C2550_CLK,		\
		.iotype		= UPIO_MEM,		\
...
static struct plat_serial8250_port stalker_gpmc_uart_data[] = {
	PORT(0x100, 5), /*dummy value need to be fix later*/
	PORT(0x108, 5),
	{ },
};

static struct platform_device stalker_gpmc_uart_device = {
	.name		= "serial8250",
	.id			= 3,
	.dev		= {
		.platform_data	= stalker_gpmc_uart_data,
	},
};

static int __init omap3_stalker_gpmc_uart_init(void)
{
 	unsigned long uart_a_base;
	unsigned long uart_b_base;
	int uart_a_cs;
	int uart_b_cs;
	int uart_a_int = 0;
	int uart_b_int = 0;
	...
	if(board_model == EGS35X_A1){
		uart_a_cs = STALKER_16C2550_CSA;
		uart_b_cs = STALKER_16C2550_CSB;
		...
		stalker_gpmc_uart_data[0].mapbase = uart_a_base;
		...
		stalker_gpmc_uart_data[1].mapbase = uart_b_base;
		
		uart_a_int = STALKER_SERIAL_GPIO_IRQ_A;
		uart_b_int = STALKER_SERIAL_GPIO_IRQ_B;
		
		stalker_gpmc_uart_data[0].irq = OMAP_GPIO_IRQ(uart_a_int);
		stalker_gpmc_uart_data[1].irq = OMAP_GPIO_IRQ(uart_b_int);
		
		omap_mux_init_gpio(uart_a_int, OMAP_PIN_INPUT_PULLUP);
		ret = gpio_request(uart_a_int, "ST16C2550 IRQ A");
		if (ret < 0) {
		...
		ret = gpio_request(uart_b_int, "ST16C2550 IRQ B");
		if (ret < 0) {
		...
		gpio_direction_input(uart_b_int);
		
		return platform_device_register(&stalker_gpmc_uart_device);
	}
	else{
		return 0;
	}
...
static struct platform_device omap3stalker_bl = {
        ...
        .id     = -1,
	.dev	= {
		.parent		= &omap3_stalker_dss_device.dev,
	},
};
...
static void __init omap3_stalker_usb_init(void)
{
	...
	if(omap3stalker_hwinfo[board_model].usb){
